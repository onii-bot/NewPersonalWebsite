...
<!-- <template>
<div>
  <h1 class="text-2xl font-extrabold text-left p-4">{{ currentSlide.title }}</h1>
  <p class="text-2xl font-thin text-left p-4">{{ currentSlide.body }}</p>
  <div class="border-2 border-purple-800 p-2 mt-5 mb-20">
    <img :class="imageClass" :src="imageSource" alt="/" />
  </div>
</div>
...
export default {
  ...
  computed: {
    ...
    imageSource() {
      return this.windowWidth > 768
        ? this.slider[this.index].desktopimage
        : this.slider[this.index].mobileimage;
    },
    imageClass() {
      return this.windowWidth > 768
        ? 'w-[1440px] h-[600px] object-cover'
        : 'w-[768px] h-[300px] object-cover'; // Adjust these classes based on your requirements
    },
  },
  methods: {
    handleResize() {
